...
export class BitbucketAPI {
  ...
  private async request<T>(method: string, path: string, body?: any, retryCount = 0): Promise<T> {
  ...
  async listPullRequests(
    ...
    return this.request<BitbucketPaginatedResponse<BitbucketPullRequest>>("GET", path);
  }
  ...
  async listRepositories(
    ...
    return this.request<BitbucketPaginatedResponse<BitbucketRepository>>("GET", path);
  }

  async getRepository(workspace: string, repoSlug: string): Promise<BitbucketRepository> {
    return this.request<BitbucketRepository>("GET", `/repositories/${workspace}/${repoSlug}`);
  }

  async getBranches(workspace: string, repoSlug: string): Promise<BitbucketPaginatedResponse<any>> {
  ...
  async getAllBranches(workspace: string, repoSlug: string): Promise<any[]> {
    const branches = [];
  ...
  async *getAllPages<T>(initialUrl: string): AsyncGenerator<T, void, unknown> {
