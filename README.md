...
- **oh-my-posh init runs fresh every shell** (upstream-documented flow, ~70ms): `eval "$(oh-my-posh init zsh --config ~/.oh-my-mu.json)"` in `.zshrc`. Each `init` generates a per-shell `POSH_SESSION_ID` and writes the config path into that session's cache; `print primary` resolves the theme from there (NOT from `POSH_THEME` at render time — that's informational only). Do NOT wrap this in `cached_eval`: a cached init freezes one session id for every shell and skips the config-priming side effect, so if that session's cache entry is ever dropped (TTL, `oh-my-posh cache clear`, upgrade) every prompt silently falls back to the built-in default theme with nothing left to re-prime it. Theme edits to `oh-my-mu.json` apply on the next shell automatically.
